...
# Public release script derived from: 2\真核病原热图.txt
...
# === 读取真核病原数据 ===
dt <- read.csv(public_file("真核病原.csv"), 
...
df_col <- data.frame(Eukaryota = c(
  rep("Parasite", 26),
  rep("Fungi", n_total_cols - 26 - 3),
  rep("Oomycetes", 3)
), stringsAsFactors = FALSE)

ha_col <- HeatmapAnnotation(
  df = df_col,
  col = list(Eukaryota = c(
    "Parasite" = "#F1B543",    # 琥珀色
    "Fungi" = "#6e86A5",       # 灰蓝色
    "Oomycetes" = "#3D9140"    # 深绿色
  )),
  show_legend = TRUE,
...
# === 输出 PDF ===
pdf(public_output("Eukaryota_Heatmap.pdf"), width = 14, height = 10)
draw(main_heatmap,
